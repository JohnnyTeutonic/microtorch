...
void test_op_gradchecks() {
    printf("=== router-op gradchecks ===\n");
    {
        ...
        float e = fd_gradcheck(
            [&](const Var& x) { return ops::mul_col(x, make_var(c0, false)); },
            randm(6, 5, 20), 10, 1);
    ...
    }
    {
        ...
        float e = fd_gradcheck(
            [&](const Var& c) { return ops::mul_col(make_var(x0, false), c); },
            randm(6, 1, 23, 0.8f), 6, 2);
        printf("  mul_col (c path)  rel err %.2e\n", e);
        CHECK(e < 5e-3f);
    }
    {
        float e = fd_gradcheck([](const Var& x) { return ops::rms_row(x); },
                               randm(5, 8, 24), 10, 3);
        printf("  rms_row           rel err %.2e\n", e);
        CHECK(e < 5e-3f);
    }
    {
        float e = fd_gradcheck([](const Var& x) { return ops::sigmoid(x); },
                               randm(5, 8, 25, 2.0f), 10, 4);
        printf("  sigmoid           rel err %.2e\n", e);
        CHECK(e < 5e-3f);
    }
    {
        float e = fd_gradcheck(
            [](const Var& x) { return ops::add_scalar(x, 0.37f); },
            randm(5, 8, 26), 10, 5);
...
void test_module_gradcheck() {
    ...
    srd.train();
    float e = fd_gradcheck([&](const Var& x) { return srd.forward(x); },
                           randm(6, 16, 30), 12, 6);
...
void test_trainability() {
    ...
    for (int step = 0; step < 12; ++step) {
        ...
        if (step == 0) { first_loss = l; first_gate = gm; }
        last_loss = l; last_gate = gm;
    }
    printf("  loss %.5f -> %.5f | mean gate %.3f -> %.3f\n", first_loss,
           last_loss, first_gate, last_gate);
