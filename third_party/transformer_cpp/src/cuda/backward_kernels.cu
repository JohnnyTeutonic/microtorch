...
__global__ void layer_norm_backward_kernel(const float* grad, const float* input,
    ...
    for (int batch_idx = tid; batch_idx < batch_size; batch_idx += stride) {
        ...
        for (int j = 0; j < hidden_size; ++j) {
            ...
            batch_sum_grad_diff += grad[batch_idx * hidden_size + j] * gamma[j] * diff;
        }
        ...
        for (int j = 0; j < hidden_size; ++j) {
            ...
            dx[idx] = gamma[j] *
                      (grad[idx] -
                       (batch_sum_grad + diff * batch_sum_grad_diff / batch_var) / hidden_size) /
                      std;
        }
    }
}
...
__global__ void gelu_backward_kernel(float* d_intermediate, const float* input, const int size) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void feed_forward_backward_kernel_2(const float* d_intermediate, const float* w1,
    ...
    for (int idx = tid; idx < total_elements; idx += blockDim.x * gridDim.x) {
        ...
        for (int k = 0; k < intermediate_size; ++k) {
            sum +=
                d_intermediate[batch * intermediate_size + k] * w1[hidden * intermediate_size + k];
        }
        dx[idx] = sum;
    }
}