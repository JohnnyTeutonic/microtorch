...
__global__ void gqa_forward_kernel(const float* query, const float* key, const float* value,
    ...
    int k_offset =
        (batch_idx * seq_len * num_kv_heads + seq_idx * num_kv_heads + kv_head_idx) * head_dim;
    ...
    for (int i = 0; i < seq_len; i++) {
        compute_attention_scores(attention_scores + threadIdx.x * seq_len, query + q_offset,
                                 key + k_offset + i * head_dim, seq_len, head_dim, i);
    }
    ...
    for (int d = 0; d < head_dim; d++) {
        float sum = 0.0f;
        for (int i = 0; i < seq_len; i++) {
            sum += attention_scores[threadIdx.x * seq_len + i] * value[v_offset + i * head_dim + d];
        }
...
void launch_gqa_kernel(const float* query, const float* key, const float* value, float* output,
    ...
    gqa_forward_kernel<<<grid_size, block_size, shared_mem_size, stream>>>(
        query, key, value, output, batch_size, num_heads, num_kv_heads, seq_len, head_dim);
}