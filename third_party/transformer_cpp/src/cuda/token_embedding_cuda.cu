...
__global__ void embedding_forward_kernel(const int* tokens, const float* embedding_table,
                                         float* output, int seq_length, int hidden_size,
                                         int vocab_size) {
...
__global__ void embedding_project_kernel(const float* input, const float* embedding_table,
                                         float* output, int seq_length, int hidden_size,
                                         int vocab_size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    ...
    for (int i = idx; i < seq_length * vocab_size; i += stride) {
        ...
        for (int j = 0; j < hidden_size; j++) {
            sum += input[seq_pos * hidden_size + j] * embedding_table[vocab_pos * hidden_size + j];
        }
...
void TokenEmbedding::forward_cuda(const std::vector<int>& tokens, Matrix& output) {
    ...
    CUDA_CHECK(cudaMalloc(&d_embedding_table, embedding_table.size() * sizeof(float)));
    ...
    CUDA_LAUNCH(embedding_forward_kernel, grid, block, 0, nullptr, d_tokens, d_embedding_table,
                d_output, seq_length, hidden_size, get_vocab_size());
...
Matrix TokenEmbedding::project_to_vocab_cuda(const Matrix& input) {
    ...
    CUDA_LAUNCH(embedding_project_kernel, grid, block, 0, nullptr, d_input, d_embedding_table,
                d_output, seq_length, hidden_size, vocab_size);
