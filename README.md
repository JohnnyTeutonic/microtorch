...
From paper to chatting model, one stack:
...
Generate the model            nn::Llama / GPT-2 / Kimi / Mamba / SRD
       ↓
Train                         mtstudio run spec.json — early stopping,
       ↓                      checkpoint/resume, JSONL telemetry
Monitor in Studio             live loss curve + per-module gradient glow
       ↓
Export GGUF / safetensors     byte-exact, vocabulary embedded
       ↓
Launch inference              tinyllama.cpp serves it
       ↓
Share the results             a run is one spec file
```

**~10K lines of core code. No CUDA required (T4-validated when you want it).
...
the docs workflow on every push to master (enable Pages → Source: GitHub
